...
%% EXAMPLE: NOMINAL
fprintf('\n=== EXAMPLE NOMINAL EVALUATION ===\n');

% Define numerical parameters (example values - adjust as needed)
params = struct();
params.m_w = 1.0;    % kg 1 rueda
params.m_b = 16.0;   % kg
params.m_m = 5.0;    % kg
params.m_a = 5.0;    % kg
params.m_f = 2.0;    % kg

params.I_w = 0.02;   % kg*m^2
params.I_b = 1.4533;    % kg*m^2
params.I_m = 0.9417;    % kg*m^2
params.I_a = 0.1531;   % kg*m^2
params.I_f = 0.03;   % kg*m^2
...
q_num = [0; 0; 0; 0];       % [x, theta_p, d_m, theta_a]
dq_num = [0; 0; 0; 0];        % velocities [dx, dtheta_p, dd_m, dtheta_a]
...
fprintf('Numerical Mass Matrix at nominal configuration:\n');
disp(M_num);

fprintf('Numerical Coriolis Matrix at nominal configuration:\n');
disp(C_num);

fprintf('Numerical Gravity Vector at nominal configuration:\n');
disp(G_num);
