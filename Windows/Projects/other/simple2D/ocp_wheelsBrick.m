...
%% model dynamics
[model, model_params, helpers] = get_wheeled_brick_model();
...
W_x = diag([1e-8, 1e2, 1e2, 1e3]);  % Nota: x tiene peso muy bajo
...
% Subplot 1: Estados
states = {'x [m]', 'β [°]', 'ẋ [m/s]', 'β̇ [°/s]'};
for i=1:nx
    subplot(3, 2, i);
    hold on; grid on;
    
    if i == 2 || i == 4
        % Convertir radianes a grados para β y β̇
        if i == 2
            ydata = rad2deg(xtraj(i,:));
        else
            ydata = rad2deg(xtraj(i,:));
        end
        plot(ts, ydata, 'b-', 'LineWidth', 2);
        ylabel(states{i});
    else
        plot(ts, xtraj(i,:), 'b-', 'LineWidth', 2);
        ylabel(states{i});
    end
...
subplot(3, 2, 5);
hold on; grid on;
stairs(ts(1:end-1), utraj(1,:), 'r-', 'LineWidth', 2);
stairs(ts(1:end-1), utraj(2,:), 'b-', 'LineWidth', 2);
ylabel('τ [Nm]');
xlabel('t [s]');
title('Torques de control');
legend('τ_L', 'τ_R', 'Location', 'best');

% Subplot 6: Trayectoria del CM
subplot(3, 2, 6);
hold on; grid on;
% Calcular trayectoria del CM
x_cm_traj = zeros(1, N+1);
z_cm_traj = zeros(1, N+1);
for i=1:N+1
    cm_pos = helpers.get_cm_position(xtraj(1,i), xtraj(2,i));
    x_cm_traj(i) = cm_pos(1);
    z_cm_traj(i) = cm_pos(2);
end
plot(x_cm_traj, z_cm_traj, 'g-', 'LineWidth', 2);
% Marcar inicio y fin
plot(x_cm_traj(1), z_cm_traj(1), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
plot(x_cm_traj(end), z_cm_traj(end), 'go', 'MarkerSize', 10, 'MarkerFaceColor', 'g');
xlabel('x_{cm} [m]');
ylabel('z_{cm} [m]');
title('Trayectoria del Centro de Masa');
legend('Trayectoria', 'Inicio', 'Fin', 'Location', 'best');
axis equal;

%% Análisis de la solución
fprintf('\n=== ANÁLISIS DE LA SOLUCIÓN ===\n');
fprintf('Estado inicial: x=%.3f m, β=%.2f°, ẋ=%.3f m/s, β̇=%.2f°/s\n', ...
    x0(1), rad2deg(x0(2)), x0(3), rad2deg(x0(4)));
fprintf('Estado final:   x=%.3f m, β=%.2f°, ẋ=%.3f m/s, β̇=%.2f°/s\n', ...
    xtraj(1,end), rad2deg(xtraj(2,end)), xtraj(3,end), rad2deg(xtraj(4,end)));

% Verificar si se alcanzó la referencia
beta_final = abs(rad2deg(xtraj(2,end)));
if beta_final < 1
    fprintf('✓ Brick estabilizado (β final < 1°)\n');
else
    fprintf('✗ Brick no completamente estabilizado (β final = %.2f°)\n', beta_final);
end
