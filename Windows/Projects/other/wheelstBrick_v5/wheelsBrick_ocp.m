...
subplot(3,1,1);
plot(ts, rad2deg(beta_traj), 'LineWidth',2); grid on;
ylabel('\beta [deg]');
title('Ángulo de pitch');

subplot(3,1,2);
plot(ts, rad2deg(beta_dot_traj), 'LineWidth',2); grid on;
ylabel('\dot{\beta} [deg/s]');
title('Velocidad angular');

subplot(3,1,3);
stairs(ts(1:end-1), tau_L, 'LineWidth',2); hold on;
stairs(ts(1:end-1), tau_R, '--', 'LineWidth',2);
grid on;
ylabel('\tau [Nm]');
xlabel('t [s]');
legend('\tau_L','\tau_R');
title('Torques en ruedas');

%% ================= SUMMARY =================
fprintf('\n=== RESULTADOS ===\n');
fprintf('Pitch inicial: %.2f deg\n', rad2deg(beta_traj(1)));
fprintf('Pitch final:   %.2f deg\n', rad2deg(beta_traj(end)));

if abs(rad2deg(beta_traj(end))) < 1
    fprintf('✓ Brick estabilizado correctamente\n');
else
    fprintf('✗ Brick no completamente estabilizado\n');
end
