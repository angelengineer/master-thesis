...
%%
% Parámetros de impedancia (solo para la dirección del movimiento F)
M_des = 10;   % Inercia Deseada (relativamente alta para estabilidad)
D_des = 20;   % Amortiguamiento Deseado (alto para absorber el golpe)
K_des = 0.5;  % Rigidez Deseada (muy baja para ceder, casi flotante)
% Nota: Estos valores son escalares porque solo controlamos F (fuerza horizontal)
% Se recomienda hacer D_des ≈ 2*sqrt(M_des*K_des) para amortiguamiento crítico.

% Variables para la integración de la nueva referencia (iniciales)
p_ref_suave = x0(1); % Empieza en la posición inicial (p)
v_ref_suave = x0(3); % Empieza con la velocidad inicial (v)
%% model dynamics
...
% define solver options for IMPLICIT integration
ocp.solver_options.N_horizon = N;
ocp.solver_options.tf = T;
ocp.solver_options.nlp_solver_type = 'SQP';
ocp.solver_options.integrator_type = 'IRK';  % CHANGED: Use IRK (Implicit Runge-Kutta) for implicit form
ocp.solver_options.sim_method_num_stages = 4;  % Number of IRK stages (typical: 2, 3, or 4)
ocp.solver_options.sim_method_num_steps = 3;   % Number of integration steps per shooting interval
ocp.solver_options.sim_method_newton_iter = 5; % Newton iterations for implicit integration
