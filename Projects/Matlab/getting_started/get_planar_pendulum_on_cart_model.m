function model = get_planar_pendulum_on_cart_model(varargin)
    % GET_PLANAR_PENDULUM_ON_CART_MODEL  AcadosModel de péndulo plano sobre carro.
    %   Todos los parámetros se fijan numéricamente en el modelo (hardcodeados).
    %
    %   MODEL = GET_PLANAR_PENDULUM_ON_CART_MODEL() usa parámetros por defecto.
    %   MODEL = GET_PLANAR_PENDULUM_ON_CART_MODEL('M', M_val, 'm', m_val, ...)
    %   permite especificar parámetros mediante pares nombre-valor.
    %
    %   Parámetros opcionales (nombre - valor):
    %       'M'      - masa del carro               (defecto: 7.67)
    %       'm'      - masa del péndulo             (defecto: 0.7)
    %       'L'      - distancia pivote → CM        (defecto: 0.155)
    %       'I'      - inercia respecto al CM       (defecto: 0)
    %       'dz'     - offset vertical del pivote   (defecto: 0) [no afecta dinámica]
    %       'dy'     - offset lateral del pivote    (defecto: 0) [no afecta dinámica]
    %       'g'      - gravedad                    (defecto: 9.81)
    %       'delta_t' - tiempo de muestreo (discretización Euler, opcional)
    %
    %   Salida:
    %       model - estructura AcadosModel con campos:
    %           .x, .xdot, .u, .z, .p (vacío)
    %           .f_expl_expr, .f_impl_expr, .disc_dyn_expr (si delta_t se da)
    %           .name = 'planar_pendulum_fixed'
    ...
    model.name = 'planar_pendulum_fixed';
end